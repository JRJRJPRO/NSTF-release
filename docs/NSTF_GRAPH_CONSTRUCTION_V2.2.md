...
> **更新日期**: 2026-02-02  
> **版本**: V2.2 (基于二次评审修订)  
> **变更摘要**: 精简属性集、移除 NoLinks 消融组、success_rate 允许 LLM 猜测、添加调试模式
...
### 3.1 CharacterResolver（多方法 Fallback）
...
class EpisodicLinker:
    ...
    def __init__(
        self, 
        verify_threshold: float = 0.25,   # 验证 LLM 指定链接
        discover_threshold: float = 0.40,  # 自动发现链接
        debug: bool = False
    ):
...
2. 运行 NSTF-Incr vs NSTF-Static
3. 测试 Step 扩展属性（triggers, outcomes）提取成功率
