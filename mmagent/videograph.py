...
class VideoGraph:
    ...
    class Node:
        def __init__(self, node_id, node_type):
            self.id = node_id
            self.type = node_type  # 'img', 'voice', 'episodic', 'semantic', or 'procedure'
            self.embeddings = []
            self.metadata = {}
    ...
    def add_procedure_node(self, procedure_info, clip_id):
        ...
        # Store procedure-specific metadata
        if 'steps' in procedure_info:
            node.metadata['steps'] = procedure_info['steps']
        if 'preconditions' in procedure_info:
            node.metadata['preconditions'] = procedure_info['preconditions']
        if 'postconditions' in procedure_info:
            node.metadata['postconditions'] = procedure_info['postconditions']
        if 'entities' in procedure_info:
            node.metadata['entities'] = procedure_info['entities']
        if 'related_node_ids' in procedure_info:
            node.metadata['related_node_ids'] = procedure_info['related_node_ids']
        ...
        logger.debug(f"Procedure node added with ID {node.id} and contents: {procedure_info['contents']}")
    ...
    def get_connected_nodes(self, node_id, type=['img', 'voice', 'episodic', 'semantic', 'procedure']):
    ...
    def search_text_nodes(self, query_embeddings, range_nodes=[], mode="max", include_procedures=True):
        ...
        if range_nodes:
            text_nodes = []
            search_types = ['episodic', 'semantic']
            if include_procedures:
                search_types.append('procedure')
            for node_id in range_nodes:
                text_nodes.extend(self.get_connected_nodes(node_id, type=search_types))
