...
# 只使用本地embedding，不调用API
from .utils.local_embedding import parallel_get_local_embedding
...
def process_memories(video_graph, memory_contents, clip_id, type='episodic'):
    def get_memory_embeddings(memory_contents):
        # 原版只使用本地embedding，不调用API（构建图谱阶段不花钱）
        embeddings = parallel_get_local_embedding(memory_contents)[0]
        return embeddings
